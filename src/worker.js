...
function detectReplyModeChange(text) {
  ...
  if (has('مش شايف', 'مش شايفه', 'مش بشوف', 'مش بعرف اقرا', 'مش عارف اقرا', 'مبعرفش اقرا', 'مش بقرا', 'نظري ضعيف', 'سمعني', 'اسمعني', 'سمعهولي', 'مش بعرف اكتب', 'مبعرفش اكتب')) return 'voice';
...
function reengageMessage(name, fromStaffPhone = false) {
  ...
  const who = !first ? 'يا فندم' : titled ? `يا ${name.split(' ').slice(0, 2).join(' ')}` : `يا ${pick(['أستاذ', 'باشمهندس'])} ${first}`;
  const buddy = first ? `يا ${pick(['حبيبي', 'غالي', 'برنس'])}` : 'يا حبيبي';

  // 1) سلام ودّ  2) عتاب خفيف  3) إن كل حاجة موجودة  4) عرض مساعدة — كل سطر ليه كذا شكل
  const hello = [
    `إزيك ${who} 👋 عامل إيه ${buddy}؟`,
    `أهلاً ${who} 🌹 أخبار معاليك إيه؟`,
    `مساء الفل ${who} 👋 عامل إيه النهاردة؟`,
    `إزيك ${who}، وحشتنا والله 🌹`,
  ];
  const miss = [
    'بقالنا كتير ما شفناكش! إيه يا عم، زعلان مننا ولا إيه؟ 😄',
    'فينك يا راجل؟ بقالك فترة ما طلّيتش علينا، إحنا مش واخدين على كده 😅',
    'إيه الغيبة الطويلة دي؟ القدس من غيرك مش حلوة والله 😄',
    'بقالك كتير ما جيتش، قلنا نطمن عليك ونسلّم 🙂',
  ];
  const stock = [
    'عندنا كل اللي إنت عايزه موجود، ونازل بضاعة جديدة بأسعار حلوة، وكله أصلي و*بالضمان*.',
    'المحل مليان بضاعة جديدة وأسعار تفرّحك، وكل حاجة أصلية و*بالضمان*.',
    'عندنا كل قطع غيار المصاعد، أسعار تجارية وأصلية و*بالضمان* زي ما إنت عارف.',
  ];
  // من تليفون موظف: الطلبات تروح لرقم المحل (عليه البوت) مش للتليفون ده
  const offer = fromStaffPhone
    ? [
        `محتاج أي حاجة؟ ابعت اسمها أو صورتها على رقم المحل تحت، وهيتردّ عليك بالسعر على طول — أمرنا ${buddy} 🙏`,
        `لو فيه أي قطعة ناقصاك أو عندك طلبية كاملة، ابعتها على رقم المحل تحت وهنسعّرهالك حالًا — تحت أمرك ${buddy} 🙏`,
        `عايزين نشوفك ${buddy} 🌹 ابعت اللي محتاجه على رقم المحل تحت (اسمه أو صورته أو مقاساته) ويوصلك السعر على طول.`,
      ]
    : [
        `محتاج أي حاجة؟ قولّي اسمها وأنا أقولك سعرها على طول، ولو عندك مقاسات ابعتها وأنا أسعّرهالك — أمرني ${buddy} 🙏`,
        `لو فيه أي قطعة ناقصاك قولّي عليها وأنا أسعّرهالك حالًا، ولو عندك طلبية ابعتهالي كلها وأنا أسعّرهالك — تحت أمرك ${buddy} 🙏`,
        `ابعتلي اللي محتاجه (اسمه أو صورته) وأنا أرد عليك بالسعر على طول — عايزين نشوفك ${buddy} 🌹`,
      ];
...
async function sendStaffMessage(agent, kind, c, message, env) {
  ...
  if (viaStaff) {
    ...
    // ويطلع "مراسلة على واتساب" — فبنكتب رقم المحل بس
    const shop = config.store.whatsapp.replace(/\D/g, '');
    // الناس بتخاف تدوس على رقم فتتصل وتدفع — فبنقول بوضوح إنه واتساب ورسايل ببلاش مش مكالمة
    const full =
      `${text}\n\n💚💬 *ده رقم واتساب المحل — للرسايل بس، مش مكالمة، وببلاش خالص:*\n` +
      `👇 دوس عليه واختار *"مراسلة"* (مش "اتصال")\n*+${shop}*\n` +
      'واسأل على أي حاجة، وفريقنا هيرد عليك على طول ❤️';
    await sendText(
      agent,
...
async function isPartsInquiry(text) {
  ...
  if (
    ...
    return true;
  // سؤال سعر، أو "عاوز/محتاج <قطعة>" — مش إجابة عادية زي "ماكينة ايطالي" (دي إجابة سؤال نوع الماكينة)
  if (!/سعر|بكام|كام|عندكم|عندك|موجود|متاح|عايز|عاوز|عوز|محتاج|اطلب/.test(a)) return false;
