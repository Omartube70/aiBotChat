...
- لما تعرض أسعار، فكّر الزبون إن **كل حاجة عندنا أصلية وبالضمان** — **مرة واحدة بس في المحادثة كلها** (جملة قصيرة في الآخر).
...
export async function generateReply(userText, history = [], { customerName } = {}) {
  ...
  // "أصلية وبالضمان" مرة واحدة بس في المحادثة — لو اتقالت قبل كده بنشيل الجملة دي من الرد الجديد
  if (reply && history.some((m) => m.role === 'model' && /ضمان/.test(m.parts?.[0]?.text || ''))) {
